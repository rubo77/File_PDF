...
class File_PDF {
	...
	var $footerInfoText = null;
	var $footerCompanyText = null;
	...
	function tableHeader($headerInfo) {
				
		foreach($headerInfo as $col) {
		
			if(empty($gesamtbreite)) $gesamtbreite = 10;
			
			$this->setxy($gesamtbreite,$col['zeile']);
			$this->Cell($col['breite'],$col['hoehe'],'',1);
			$this->setxy($gesamtbreite,$col['zeile']);
			$this->MultiCell($col['breite'],($col['hoehe']/2),$col['text'],'0','L');
			$this->setxy($gesamtbreite+$col['breite'],$col['zeile']);
			
			$gesamtbreite += $col['breite'];
					
		}
	
		$this->sety($col['zeile']+$col['hoehe']);
     ...
     */
    static function &factory($params = array(), $class = 'File_PDF')
    {
        /* Default parameters. */
    ...
    function footer()
    {
       // Go to 1.5 cm from bottom 
				...
				// Print Company-Info if set
				if(!empty($this->footerCompanyText)) {
					$this->line($this->_left_margin, -15, $this->_right_margin, -15);
					$this->cell(0, 5, $this->footerCompanyText, 0, 0, 'C');
					$this->newline();
				}
				
				// Print centered page number 
				$this->setFont('Arial', 'I', 9);
				$this->cell(0, 3, '[ '.$this->footerInfoText.$GLOBALS['PROGRAM_NAME'].' - Seite ' . $this->getPageNo() . ' von {nb} ]', 0, 0, 'C');
    }
    ...
    function output($filename = 'unknown.pdf', $inline = false)
    {
        ...
        if (!include_once 'vendors/pear/HTTP_Download/Download.php') {
    ...
    function _getFontFile($fontkey, $path = '')
    {
        ...
        if (!isset($font_widths[$fontkey])) {
            if (!empty($path)) {
                ...
                $file = 'vendors/pear/File_PDF/PDF/fonts/' . strtolower($fontkey) . '.php';
            }
            include $file;
            if (!isset($font_widths[$fontkey])) {
